...
export const GET = async (req: NextRequest) => {
	...
	const recommendedMatrix: number[][] = [];
	for (let i = 0; i < recommended.length; i++) {
		const split = recommended[i].split(",").map(x => parseInt(x));
	...
	const result: any =
		await prisma.$queryRaw`SELECT embedding::text FROM "Recommendation" WHERE id != ${user_id} ORDER BY embedding <-> ${r_embedding}::vector LIMIT 3;`;

	// Get recommended posts
	let userLikes: number[] = [];
	const user_recommend: number[] = []; // All current user likes
